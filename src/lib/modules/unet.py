...
class UNetUpBlock(nn.Module):
    ...
    def center_crop(self, layer, target_size):
        _, _, layer_x, layer_y, layer_z = layer.size()
        diff_x = (layer_x - target_size[0]) // 2
        diff_y = (layer_y - target_size[1]) // 2
        diff_z = (layer_z - target_size[2]) // 2
        return layer[
            :, :, diff_x : (diff_x + target_size[0]) ,diff_y : (diff_y + target_size[1]), diff_z : (diff_z +
                                                                                                 target_size[2])
        ]
