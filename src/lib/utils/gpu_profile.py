...
def set_gpu_profile_fn(root):
    global gpu_profile_fn
    gpu_profile_fn = os.path.join(root, f"{datetime.datetime.now():%d-%b-%y-%H:%M:%S}-gpu_mem_prof.txt")
...
def _trace_lines(frame, event, arg):
    ...
    where_str = f"{func_name} in {filename_short}:{line_no}"
    with open(gpu_profile_fn, 'a+') as f:
        f.write(f"{where_str:<50} --> {mem_used:<7.1f}Mb [{code_line.rstrip()}] \n")
