...
def main(config):
    ...
    # For every pair of worms, read the corresponding two gt files. only keep consistent
    ...
    for i in range(num_worms-1):
        ...
        for j in range(i+1, num_worms):
            ...
            tmp_w2w_cpm = dict()
            for w1shid, w1sh in enumerate(w1_seghyplnames):
                if not ulabels.is_valid_label(w1sh):
                    continue
                if not w1sh in w2_seghyplnames:
                    continue
                w2shid = w2_seghyplnames.index(w1sh)
                tmp_w2w_cpm[w1shid+1] = w2shid+1  # I think we need +1 as labels start from 1 and 0 reserved for bg
            ...
            # Evaluation or not
            if config['general']['evaluate']:
                logging.info(f'w2w_cpm:{w1uid}-{w2uid}:'
